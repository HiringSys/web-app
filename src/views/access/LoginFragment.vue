...
const buttonText = computed(() => step.value === 'email' ? 'Continuar' : 'Entrar')
...
<template>
  <div class="flex flex-col w-full gap-6">
    <div class="flex flex-col gap-2">
      ...
      <form class="w-full max-w-120 flex flex-col gap-4" @submit.prevent="submit">
        <Transition name="field-swap" mode="out-in">
          ...
          <Input v-else key="password" v-model="password" type="password" placeholder="Senha" />
        </Transition>
    ...
    <p v-if="step === 'email'" class="font-medium text-black/90">
      ...
      <HintPopup message="Peça ao responsável de RH ou TI da sua empresa para provisionar o seu acesso.">Fale com sua empresa</HintPopup>
    </p>
    <p v-else class="font-medium text-black/90">
      Não lembra sua senha? <RouterLink :to="{ name: 'recuperar-senha' }" class="text-blue cursor-pointer">Recuperar</RouterLink>
    </p>
  </div>
</template>

...
import {
  parseFuncionariosSheet,
  type ParsedFuncionariosImport,
...
async function handleFileChange(event: Event) {
  ...
  try {
    ...
    const sheet =
      sheets.find((s) => s.sheet.toLowerCase() === "funcionarios") ?? sheets[0];
...
async function confirmImport() {
  if (
    !parsed.value ||
    parsed.value.funcionarios.length === 0 ||
    submitting.value
  )
    return;
  ...
  try {
    const result = await importFuncionariosFromExcel(
      props.grupoId,
      parsed.value.funcionarios,
    );
    notify(
      result.mensagem ??
        `${result.totalRecebidos ?? parsed.value.funcionarios.length} candidato(s) importado(s).`,
      "success",
    );
    emit("imported");
    close();
    ...
    notify(
      reason
        ? `Não foi possível importar a planilha: ${reason}`
...
<template>
  <Popup
    ...
    title="Importar candidatos via Excel"
    width="32rem"
    @update:model-value="
      (value) => {
        if (!value) close();
  ...
  >
    <div class="flex flex-col gap-4 pb-2">
      ...
      />
      <Button
        :text="fileName || 'Selecionar planilha'"
        :variant="!fileName ? 'neutral' : 'primary'"
        :color="!fileName ? 'green' : 'white'"
        :disabled="parsing"
        @click="pickFile"
      />
      ...
      <template v-else-if="parsed">
        <p class="font-semibold">
          {{ parsed.funcionarios.length }} de {{ parsed.totalRows }} linha(s)
          escaneadas com sucesso. Clique em importar para prosseguir!
        </p>

        <div
          v-if="parsed.issues.length"
          class="flex max-h-40 flex-col gap-1 overflow-y-auto rounded-medium bg-white p-3"
        >
          <span
